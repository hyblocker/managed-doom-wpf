...
namespace MeltySynth
{
    ...
    public sealed class MidiFile
    {
        ...
        internal class MidiChunk {
            internal List<Message> Messages;
            internal List<int> Ticks;

            public MidiChunk(List<Message> messages, List<int> ticks) {
                this.Messages = messages;
                this.Ticks = ticks;
            }

            public void Deconstruct(out List<Message> message, out List<int> tick) {
                message = Messages;
                tick = Ticks;
            }
        }

        internal class MidiTrackData {
            internal Message[] Messages;
            internal TimeSpan[] Timings;

            public MidiTrackData(Message[] messages, TimeSpan[] timings) {
                this.Messages = messages;
                this.Timings = timings;
            }

            public void Deconstruct(out Message[] message, out TimeSpan[] timing) {
                message = Messages;
                timing = Timings;
            }
        }
        ...
        private static MidiChunk ReadTrack(BinaryReader reader, MidiFileLoopType loopType)
        {
            var chunkType = reader.ReadFourCC();
            if (chunkType != "MTrk")
            {
                    ...
                    case 0xFF: // Meta Event
                        switch (reader.ReadByte())
                        {
                            case 0x2F: // End of Track
                                ...
                                ticks.Add(tick);
                                return new MidiChunk(messages, ticks);
        ...
        private static MidiTrackData MergeTracks(List<Message>[] messageLists, List<int>[] tickLists, int resolution)
        {
            ...
            return new MidiTrackData(mergedMessages.ToArray(), mergedTimes.ToArray());
        }
