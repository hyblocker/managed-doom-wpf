...
namespace MeltySynth
{
    internal sealed class Voice
    {
        ...
        public void Start(RegionPair region, int channel, int key, int velocity)
        {
            ...
            instrumentPan = MathExtensions.Clamp(region.Pan, -50F, 50F);
        ...
        public bool Process()
        {
            ...
            if (dynamicCutoff)
            {
                ...
                smoothedCutoff = MathExtensions.Clamp(newCutoff, lowerLimit, upperLimit);
            ...
            var angle = ((float)Math.PI / 200F) * (channelInfo.Pan + instrumentPan + 50F);
            if (angle <= 0F)
            {
            ...
            currentReverbSend = MathExtensions.Clamp(channelInfo.ReverbSend + instrumentReverb, 0F, 1F);
            currentChorusSend = MathExtensions.Clamp(channelInfo.ChorusSend + instrumentChorus, 0F, 1F);
