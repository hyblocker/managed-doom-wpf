...
namespace MeltySynth
{
    ...
    {
        private static readonly float resonancePeakOffset = 1 - 1 / (float)Math.Sqrt(2);
        ...
        public void SetLowPassFilter(float cutoffFrequency, float resonance)
        {
            if (cutoffFrequency < 0.499F * synthesizer.SampleRate)
            {
                ...
                var w = 2 * (float)Math.PI * cutoffFrequency / synthesizer.SampleRate;
                var cosw = (float)Math.Cos(w);
                var alpha = (float)Math.Sin(w) / (2 * q);
