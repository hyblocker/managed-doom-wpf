...
namespace ManagedDoom
{
	public sealed class DoomGame
	{
		...
		{
			options.Skill = (GameSkill) MathExtensions.Clamp((int)skill, (int)GameSkill.Baby, (int)GameSkill.Nightmare);

			if (options.GameMode == GameMode.Retail)
			{
				options.Episode = MathExtensions.Clamp(episode, 1, 4);
			}
			...
			if (options.GameMode == GameMode.Commercial)
			{
				options.Map = MathExtensions.Clamp(map, 1, 32);
			}
			else
			{
				options.Map = MathExtensions.Clamp(map, 1, 9);
			}
