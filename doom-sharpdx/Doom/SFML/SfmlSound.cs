    ...
    public sealed class SfmlSound : ISound, IDisposable
    {
        ...
        public SfmlSound(Config config, Wad wad)
        {
            try
            {
                ...
                config.audio_soundvolume = MathExtensions.Clamp(config.audio_soundvolume, 0, MaxVolume);
        ...
        private void SetParam(Sound sound, ChannelInfo info)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    sound.Position = new Vector3(( float ) Math.Cos(angle), ( float ) Math.Sin(angle), 0);
